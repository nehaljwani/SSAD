...
while ($row=mysql_fetch_row($result)){                 //fetching rows from result query
	?>
		...
		foreach($row as $col){        //for columns printing purposes
			if($i==0){
				//putting Del button in first column for deleting requests ,put referencing link in this href=" ";
			?>
			<td><?php echo $col;?> <a href='request.php' style='text-align:right'>Del</a> 
		</td><?php
		}
		else {
			?>
